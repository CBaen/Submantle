...
**The conflict:** Team 2 states flatly: "Submantle cannot be a process-monitoring daemon on iOS. The platform's sandboxing and background execution model makes this architecturally impossible." Team 3 lists Apple Vision Framework as accessible on iOS, citing it as "Excellent for macOS/iOS Submantle clients" for presence detection and body pose tracking.
...
**The actual iOS story:** iOS Submantle can use the camera for person detection while the app is in the foreground. It cannot enumerate processes. It cannot run continuously in the background. It cannot detect what applications are active. The Vision Framework's accessibility does not change iOS's fundamental constraint for Submantle's core value proposition — which is OS-level process awareness. A foreground-only camera presence sensor is not the same thing as ambient background awareness. Team 3 should not list Apple Vision as an iOS path for Submantle without that qualification. **Team 2's position is correct on the architectural constraint; Team 3 is not wrong but is misleading by omission.**
...
**Verdict: Team 5 has stronger evidence on the legal constraints. Team 3's WiFi optimism is not technically wrong but is legally incomplete.** The practical WiFi sensing capability for Submantle v1 is significantly more limited than Team 3's synthesis suggests.
...
1. **Kernel components require code signing certificates** (Windows kernel drivers, macOS System Extensions). Open-sourcing the code does not mean users can compile and run the kernel components themselves — they still need Submantle's signed binaries. This creates a partial open-source story that may confuse users about what they can actually audit and build themselves.

2. **eBPF policy files that define enforcement rules are the actual attack surface.** If Submantle open-sources the policy engine, any attacker can study exactly which syscall patterns Submantle blocks and craft exploits that bypass them. This is less of an issue for a context broker (Submantle doesn't claim to be a security product in the adversarial sense) but becomes relevant if enterprises rely on Tier 3 enforcement for actual security guarantees.
...
Every team references Submantle's "user intent model" as a core capability that creates moat. No team researched what building such a model actually requires: training data volume, ML architecture, inference latency, model update cadence, personalization approaches. Team 3 touches on on-device LLMs for sensor fusion but only at the 7-8B model level, which Team 3 itself notes requires 8-16GB RAM and is "viable on developer machines, not on minimal edge nodes yet." The intent model — arguably Submantle's most defensible long-term moat — is treated as a product feature to be built, not a research question to be answered.
...
Teams 1, 4, and 6 all independently conclude that MCP is the standard. Team 4 provides the most technical depth; Teams 1 and 6 provide market validation. The convergence is strong: Submantle as MCP server is the lowest-friction integration path with the broadest reach. This is a reliable finding.
...
2. What is the liability exposure when Submantle's safety assessment is wrong?
...
5. What is Submantle's EU AI Act risk tier, and does the August 2026 deadline apply?
