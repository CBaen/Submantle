...
Before assessing which model fits Submantle, it's essential to understand the structural reality of each reference protocol — not the simplified version.
...
**2025 developments:** MCP (Model Context Protocol) November 2025 spec mandates OAuth 2.1 for all remote server authentication. MCP servers are now classified as OAuth Resource Servers, meaning they advertise protected resource metadata. This is directly relevant to Submantle's MCP surface.
...
## Section 2: Topology Analysis for Submantle's Constraints
...
**Why it doesn't fit Submantle:**
- P2P reputation propagation (EigenTrust, GossipSub scoring) requires nodes to share their local scores with neighbors. This is privacy-hostile: Submantle's local trust computation is a feature, not a limitation. Broadcasting behavioral data to unknown peers violates "privacy by architecture."
...
**What P2P gets right:** The local computation model. BitTorrent clients track peer quality locally. This mirrors Submantle's model exactly — trust is computed on your device, for your use.
...
**What this means for Submantle:** Pure federation isn't viable. The trust data is on-device. There's no "Submantle server" that could be federated. The protocol needs to support verification across instances without requiring the computation to move off-device.
...
**What hub-and-spoke gets right for Submantle:**
...
**What this means for Submantle:** Submantle could use a thin coordination layer (Submantle's own servers) for agent registration, credential issuance, and trust score attestation — while keeping all computation on-device. The coordination server never touches behavioral data. It only issues signed attestations about what a device has computed.

### 2.4 The Hybrid That Actually Matches Submantle's Requirements

The architecture that fits all five of Submantle's constraints is not any single topology — it's a split-plane design pioneered by Tailscale:

**Control plane (thin, hub-and-spoke):** Handles registration, identity verification, attestation issuance. Submantle's servers receive "this agent has score X, computed on device Y" and issue a W3C VC (SD-JWT) asserting that fact. The server is a notary, not a data processor. It never sees behavioral data.
...
**Verification layer (federated, peer-to-peer):** Brands and platforms that receive an agent's VC validate it cryptographically against Submantle's public key. No callback to Submantle required. This mirrors how TLS certificate validation works — the CA (Submantle) signs the credential, the verifier checks the signature locally.

This is structurally identical to the TLS model applied to behavioral trust:
- CA → Submantle attestation service
...
- **Fits our case because:** Submantle's trust computation is the data plane — it must stay on-device. Attestation issuance is the control plane — it's a narrow, rare operation that can involve Submantle's servers without creating a privacy problem.
...
- **Fits our case because:** Submantle's "Submantle Store" for identity signatures is a perfect fit for DNS-style hierarchical resolution. Signatures are authoritative data at the edges (community contributors). The central server holds only pointers and metadata. Clients cache signatures locally — this is what happens today with the signatures.json file.
- **Tradeoffs:** DNS has significant governance complexity (ICANN, TLD operators). Submantle doesn't need that level of governance initially, but the architecture should be designed for it from the start.
...
- **Fits our case because:** This is exactly what Submantle's W3C VC (SD-JWT) model is. The attestation server issues a signed credential. Brands validate locally. Submantle is not in the critical path of every trust check. This is the settled architectural decision — this research confirms it's the correct one.
...
- **Fits our case because:** This is the architectural proof that "always aware, never acting" works at internet scale. DMARC provides the signal; receivers enforce. Submantle provides the trust score; brands enforce. The email trust stack is the direct precedent. This should be in Submantle's pitch materials.
- **Tradeoffs:** DMARC took a decade to reach majority adoption after SPF was deployed. The trust signal must reach critical mass before it changes behavior. Submantle faces the same cold start problem.
...
- **What:** Apply the exact Tailscale architectural split to trust: a thin Submantle coordination server handles only attestation issuance (like Tailscale's coordination server handles only key exchange). On-device daemon handles all computation (like WireGuard handles all actual traffic).
...
- **What:** CT logs are append-only Merkle trees that any browser can audit. Gossip protocols detect if a CA is serving different views to different clients. Apply this to Submantle: a public append-only log of issued trust attestations, with gossip verification to detect if Submantle is behaving inconsistently.
...
### Novel #3: IETF EAT (Entity Attestation Token) as Submantle's Credential Format Extension
...
- **Why it's interesting:** Submantle's W3C VC (SD-JWT) is the right format for portable credentials. EAT could be the format for the *claims* inside those credentials — giving Submantle's attestations a standard vocabulary that IETF is already defining.
- **Evidence:** IETF RATS WG is active (multiple drafts in 2025-2026). The agentic EAT draft expired December 2025 but reflects real standardization momentum. The dynamic attestation draft (draft-jiang-seat-dynamic-attestation) addresses long-lived sessions where agent runtime posture changes frequently — directly relevant to Submantle's ongoing behavioral monitoring.
...
- **Fits our case because:** Submantle could define its behavioral trust claims using EAT vocabulary and wrap them in SD-JWT VCs. This positions Submantle as standards-aligned from day one — not proprietary. Agents carrying Submantle credentials would be speaking a language that IETF is already standardizing.
- **Risks:** EAT drafts are still in early stages. The agentic extension is one team's proposal, not yet consensus. Submantle should track but not depend on any specific draft. The W3C VC wrapper is more stable than the claim vocabulary.
...
- **Momentum:** 13,000+ MCP servers launched on GitHub in 2025. 97M+ monthly SDK downloads (per Submantle's prior research). MCP is becoming the dominant agent-to-tool integration layer. The November 2025 spec explicitly anticipates "enterprise registries and federated trust frameworks."
...
- **Fits our case because:** Submantle's MCP server can present itself as an OAuth Resource Server. Agents querying Submantle over MCP can present their Submantle trust credentials as OAuth tokens. This makes trust verification a native part of the MCP handshake, not a separate step. The "ambient stream" Submantle vision — where agents feel the ground without asking — becomes the MCP subscription model.
...
- **Fits our case because:** Submantle's portable W3C VC credentials are exactly the architecture CSA is calling for. The revocation mechanism (Bitstring Status List) provides the coordinated revocation. Submantle is positioned to be the answer to the question CSA is asking publicly.
- **Maturity risk:** No standard yet defines how cross-domain agent revocation works. The CSA framework is analysis, not specification. Submantle building this now would be ahead of any standard — which is a moat opportunity but also a standard convergence risk.
...
- **Momentum:** ZK proofs for credential verification are gaining traction in the W3C VC community. SD-JWT (which Submantle has adopted) provides selective disclosure without ZK — but ZK provides a stronger privacy guarantee (no information leakage even from what's disclosed).
...
3. **The cold start / bootstrap problem for brands.** A brand that wants to query Submantle trust scores needs to trust Submantle's attestation. Who establishes that trust? In TLS, browser vendors establish root CA trust. In Submantle, there's no equivalent trust anchor decision authority. This is a governance decision, not a technical one, but it blocks the brand API business.
...
5. **Whether IETF agent identity drafts will converge on compatible formats.** There are at least 6 IETF drafts active in this space. They may conflict. Submantle should monitor but not depend on any single draft. The right move is to be VC/SD-JWT-based (stable W3C standard) and add IETF-compatible claim vocabulary when standards converge.

---

## Section 7: Synthesis

### The Strongest Architecture for Submantle

**Submantle is a Tailscale-for-trust:** split-plane design where computation is on-device and coordination is a thin server. This maps exactly to Submantle's constraints:

| Plane | What it does | Submantle equivalent |
|-------|-------------|---------------------|
| Control plane (thin, centralized) | Agent registration, attestation issuance, signature distribution | Submantle servers — thin notary |
| Data plane (on-device, never centralized) | Trust score computation, behavioral history, process awareness | Local daemon + SQLite |
| Verification layer (stateless, distributed) | Brand/platform validation of trust credentials | Cryptographic signature check against Submantle public key |
...
- **OAuth JWTs:** Stateless credential validation, brands don't need to call Submantle on every check
...
**Phase 1 (now → MCP server):** On-device daemon (existing), REST/MCP API, local trust computation, JWT-style token issuance from Submantle servers. Architecture: Tailscale control plane model. No gossip, no federation, no P2P.

**Phase 2 (trust layer wired → brand API):** W3C VC attestation issuance. Brands validate Submantle VCs without calling back to Submantle. Revocation via Bitstring Status List. Architecture: TLS/CA model applied to behavioral trust. DMARC model for "signal without enforcement."
...
1. **Submantle is architecturally a TLS Certificate Authority for behavioral trust.** This is the most precise technical analogy: Submantle's on-device daemon is the client; the attestation server is the CA; brands are the browsers; behavioral trust score is the certificate. Everything maps. The DMARC analogy explains the enforcement model. Use both.

2. **The biggest architectural risk is not topology — it's the cold start / trust anchor problem.** Who decides to trust Submantle's root signing key? In TLS, browser vendors make that decision. Submantle needs an equivalent governance layer. This is a business and ecosystem problem, not a technical one. Without it, the brand API business doesn't work.

3. **PGP is the cautionary tale.** Web of Trust failed because it asked too much of ordinary people, had no forward secrecy, and required human key signing parties that never achieved critical mass. Submantle must not replicate PGP's failure modes: it must be invisible to operate (agents register automatically), the UX must require nothing of device owners, and trust must accumulate passively.

4. **ActivityPub's failure modes are Submantle's design checklist.** Each failure mode maps to a Submantle architectural decision already made or pending:
   - Identity loss on server failure → on-device storage means agents own their history
   - Economic sustainability → Submantle's revenue model (API fees, subscriptions) creates sustainability incentives
   - Defederation as only enforcement → "always aware, never acting" means Submantle never needs defederation
   - Centralization by default → thin coordination server, not a host for user data

5. **The IETF space is moving fast and Submantle is ahead of it.** The six active IETF drafts on agent identity (2025-2026) are defining requirements, not solutions. They all acknowledge the behavioral trust gap. Submantle has a working prototype while IETF has working drafts. First-mover with a working implementation is a meaningful position.

6. **MCP is the right integration surface, and the November 2025 spec upgrade makes it more so.** MCP servers are now OAuth Resource Servers. Submantle can issue OAuth 2.1-compatible tokens as part of its trust attestation. Agents querying Submantle over MCP naturally get trust-checked as part of the protocol handshake. The integration surfaces align.
...
### 8.1 RATS Architecture (RFC 9334) — The Precise Technical Name for What Submantle Is

The IETF RATS working group (Remote ATtestation ProcedureS) has produced RFC 9334, finalized as an Internet Standard. This RFC defines the exact architectural vocabulary for what Submantle is building.

**The three RATS roles map directly to Submantle:**

| RATS Role | What It Does | Submantle Equivalent |
|-----------|-------------|---------------------|
| **Attester** | Produces evidence about its own state | The on-device Submantle daemon — it computes the trust score locally and generates evidence |
| **Verifier** | Appraises evidence using reference values, produces attestation results | Submantle's attestation server — receives the computed score + cryptographic evidence, signs and issues a VC |
...
1. **Passport Model:** The Attester gets an attestation result from the Verifier, then presents it directly to Relying Parties. This is exactly Submantle's W3C VC model — the agent carries its credential and presents it anywhere.
2. **Background-Check Model:** The Relying Party forwards evidence directly to the Verifier. This is the alternative model (brand calls Submantle API in real-time).

**Why this matters:** Submantle should use the Passport Model. Agents carry credentials. Brands validate locally. Submantle's attestation server is only in the path at issuance time, not at verification time. This is what the prior findings call "stateless verification" — RATS gives it a standard name.

The RATS WG has active drafts for Attestation Results for Secure Interactions (ar4si), Endorsed Attestation Results (ear), and Epoch Markers. These are evolving toward exactly the vocabulary Submantle needs.
...
- **Certificate stapling analog:** Holders can provide a cached copy of the status list VC directly to verifiers ("stapling"), further insulating from Submantle tracking who requests revocation checks.

**For Submantle:** The revocation infrastructure is a CDN-hosted VC — low operational cost, privacy-preserving by design. The "who watches the watchman" risk is mitigated by the holder-stapling mechanism. This is a solved problem. The prior expedition's Risk #2 (VC revocation unsolved) can be downgraded — the mechanism exists and the privacy tradeoffs are acceptable.
...
**Credential flow:**
1. Issuer signs and delivers VC to Holder (Submantle attestation server → Agent's local storage)
2. Holder presents VC or derived VP to Verifier (Agent → Brand's API)
3. Verifier validates signature against Issuer's public key (Brand checks Submantle's public key — can be hardcoded or resolved via DID document)

This confirms the prior findings. Adding the explicit confirmation that Step 3 requires no callback to Submantle — brands can cache Submantle's public key and verify entirely offline.
...
NOSTR (Notes and Other Stuff Transmitted by Relays) is a minimalist federated protocol not covered in the prior findings. It introduces a pattern that may be relevant to Submantle's trust attestation distribution:
...
**Why relevant to Submantle:**
- Trust attestation VCs could be distributed via a NOSTR-like relay network. An agent's trust credential could be published to multiple Submantle-compatible relays. Brands query any relay. No single relay is critical.
- Operational cost is minimal — a relay runs on a $5/month server. This is a meaningful data point for Submantle's future infrastructure scaling.
- The NOSTR model separates identity (the cryptographic key) from hosting (the relay). This mirrors Submantle's DID-based identity model.

**Fit assessment:** This is a V3+ consideration for distributing trust attestations in a resilient, decentralized way. It does not replace the control plane (attestation issuance requires Submantle's signing key). But it reduces the reliance on Submantle's attestation distribution infrastructure long-term.
...
Ink & Switch's local-first software principles (2019, with active follow-up work through 2025) define seven ideals that directly map to Submantle's design. Verifying these against Submantle's current architecture:

| Ideal | Submantle Status |
|-------|-----------------|
...
**Why this maps to Submantle's "always aware, never acting" principle:**

Submantle's trust computation is the speech layer — it observes and scores without enforcement. Brands and platforms are the reach layer — they decide what to do with the signal. The AT Protocol model shows this isn't just a design choice Submantle made; it's an architectural pattern that a well-funded (if not yet profitable) decentralized protocol team independently converged on.

The direct analogy: AT Protocol's PDS (Personal Data Server) is like Submantle's on-device daemon. AT Protocol's Relay is like Submantle's attestation aggregation (thin, handles distribution). AT Protocol's AppView is like each brand's trust enforcement layer.
...
**For Submantle:** Submantle's MCP server needs to be configured explicitly in each agent's client configuration. The "ambient stream" (agents feel the ground without asking) vision requires agents to discover and connect to Submantle automatically. This is not possible under the current MCP spec — there is no auto-discovery mechanism.

**Implication:** Submantle may need to develop or advocate for an MCP server auto-discovery standard. In the near term, the Submantle daemon could auto-inject itself into local MCP client configurations on installation — analogous to how antivirus software installs system-level hooks without user configuration. This is architecturally feasible but requires platform-specific implementation (Windows registry, macOS LaunchDaemon config injection).
...
**Submantle anti-patterns derived from PGP's failure:**
...
| **Submantle (proposed)** | **On-device daemon + thin attestation server + public key** | **Yes, for attestation issuance only** | **On-device (computation) + signed VCs (portability)** |

The table confirms: Submantle's minimum infrastructure is among the lightest of any trust protocol. A single signing key, an HTTPS endpoint for attestation, and the on-device daemon. Everything else (brand validation, agent registration, multi-device sync) can be layered on incrementally.
